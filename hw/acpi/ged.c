...
static uint64_t ged_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    case ACPI_GED_IRQ_SEL_OFFSET:
        ...
        break;
    case ACPI_OPREG_MNAD_OFFSET:
        val = ged_st->msi_min_addr;
        break;
    case ACPI_OPREG_MXAD_OFFSET:
        val = ged_st->msi_max_addr;
        break;
                      ...
                      unsigned int size)
{
    ...
    switch (addr) {
    case ACPI_OPREG_MNAD_OFFSET:
        ged_st->msi_min_addr = data;
        break;
    case ACPI_OPREG_MXAD_OFFSET:
        ged_st->msi_max_addr = data;
        break;
    case ACPI_OPREG_MNDT_OFFSET:
                   ...
                   GedEvent *events, uint32_t events_size)
{
    ...
    field = aml_create_qword_field(aml_name(AML_BUF_MSI_RESOURCE),
                                   aml_int(0x3), AML_BUF_MSI__MNA_FLD);
    aml_append(crs, field);
    field = aml_create_qword_field(aml_name(AML_BUF_MSI_RESOURCE),
                                   aml_int(0xb), AML_BUF_MSI__MXA_FLD);
    aml_append(crs, field);
    field = aml_create_qword_field(aml_name(AML_BUF_MSI_RESOURCE),
    ...
    aml_append(crs, field);
    aml_append(crs, aml_store(aml_name(AML_GED_MSI_MIN_ADDR),
                              aml_name(AML_BUF_MSI__MNA_FLD)));
    aml_append(crs, aml_store(aml_name(AML_GED_MSI_MAX_ADDR),
                              aml_name(AML_BUF_MSI__MXA_FLD)));
    ...
    field = aml_create_qword_field(aml_arg(0), aml_int(0x3),
                                   AML_BUF_MSI__MNA_FLD);
    aml_append(srs, field);
    field = aml_create_qword_field(aml_arg(0), aml_int(0xb),
                                   AML_BUF_MSI__MXA_FLD);
    aml_append(srs, field);
    field = aml_create_qword_field(aml_arg(0), aml_int(0x13),
                                   AML_BUF_MSI__MND_FLD);
    aml_append(srs, field);
    field = aml_create_qword_field(aml_arg(0), aml_int(0x17),
                                   AML_BUF_MSI__MXD_FLD);
    aml_append(srs, field);
    aml_append(srs, aml_store(aml_name(AML_BUF_MSI__MNA_FLD),
                              aml_name(AML_GED_MSI_MIN_ADDR)));
    aml_append(srs, aml_store(aml_name(AML_BUF_MSI__MXA_FLD),
                              aml_name(AML_GED_MSI_MAX_ADDR)));
